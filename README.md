...
## Build from source (AMO source submission)

Everything needed to reproduce the exact submitted zip is in this repository.
**No transpilation, concatenation, or minification anywhere** — the shipped
JavaScript is the source JavaScript (plain ES5 script tags).

### Environment requirements

| Tool | Version | Install |
|---|---|---|
| Node.js + npm | ≥ 18 (LTS 20/22 recommended) | https://nodejs.org, or nvm |
| `zip` | any recent (≥ 3.0) | macOS: built-in · Debian/Ubuntu: `sudo apt install zip` · Windows: WSL or Git Bash |

The extension itself has zero npm dependencies — no `npm install` is ever needed.

### Step-by-step build

```bash
git clone https://github.com/FirozChauhan/Glisters.git
cd Glisters
node --version          # must be ≥ 18

# 1. (recommended) Regenerate config/auth/icons from .env
cp .env.example .env    # fill in R2_WORKER_URL etc.
npm run build           # → js/config.js, js/auth.js, icons/*.png

# 2. Build the AMO zip (the only step strictly required)
npm run build:firefox   # → dist/glisters-firefox-<version>.zip

# 3. (optional) Validate exactly like AMO does
npm run lint:firefox    # web-ext lint — 0 errors
```

Step 2 alone reproduces the shipped zip: `js/config.js` and `js/auth.js` are
**committed** (public-scope values only), so a fresh clone already contains the
exact shipped files — step 1 only refreshes timestamps/values.

### Notes on the generated files

- `js/config.js` — generated by `scripts/gen-config.mjs` from `.env`
  (public-scope values only: worker URL + Clerk publishable key).
- `js/auth.js` — byte-identical copy of `js-src/auth.js` plus a one-line
  generated-at banner (`scripts/gen-auth.mjs`). Edit `js-src/auth.js`, never
  `js/auth.js`.
- `icons/icon{16,48,128}.png` — generated by `scripts/gen-icons.mjs`
  (deterministic, zero image dependencies).
- Nothing is minified, bundled, transpiled, or obfuscated.
