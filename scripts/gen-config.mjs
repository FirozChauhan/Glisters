...
const cfg = {
  ...
  /* optional override: reachable Clerk frontend API when the publishable key
     embeds a dead/custom domain — auth.js derives the real domains from the
     pk and falls back to this (see js-src/auth.js) */
...
console.log('clerk proxy url: ' + (cfg.clerkProxyUrl || 'NOT SET (auth.js derives domains from the publishable key)'));
